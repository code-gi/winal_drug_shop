"""
Script to force populate the PostgreSQL database with sample medications.
This script ignores existing record checks and forces a clean population.
"""
...
def force_populate_db():
    with app.app_context():
        ...
        # Create categories - using the same code as in populate_db.py
        ...
        for idx, cat_data in enumerate(human_categories, 1):
        ...
        for idx, cat_data in enumerate(animal_categories, 101):
            category = Category(**cat_data)
            db.session.add(category)
            db.session.flush()  # Get the ID without committing
            category_map[cat_data["name"]] = category.id
        ...
        # Human and animal medications - copy from your populate_db.py
        # Copy the medications data from your existing populate_db.py script
        ...
        print("Creating medication images...")
        images = [
